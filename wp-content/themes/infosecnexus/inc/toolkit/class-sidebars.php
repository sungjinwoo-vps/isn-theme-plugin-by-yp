			...
			register_sidebar(
				array(
					'name'          => sprintf(
						...
						__( 'InfoSecNexus Conditional Sidebar %d', 'infosecnexus' ),
						$index
					),
					'id'            => 'infosecnexus-conditional-' . $index,
					'description'   => __( 'Rendered when its JSON condition set matches the current request.', 'infosecnexus' ),
	...
	public static function render(): bool {
		...
		foreach ( $sets as $set ) {
			...
			if ( Conditions::matches( $set['conditions'] ?? array() ) ) {
				echo '<aside id="secondary" class="widget-area widget-area--conditional" aria-label="' . esc_attr__( 'Conditional sidebar', 'infosecnexus' ) . '">';
