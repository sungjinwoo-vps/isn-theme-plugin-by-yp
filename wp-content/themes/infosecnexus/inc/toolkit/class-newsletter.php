...
final class Newsletter {
	...
	public static function subscribe( WP_REST_Request $request ): WP_REST_Response {
		if ( ! rate_limit( 'newsletter', 5, 300 ) ) {
			return new WP_REST_Response( array( 'message' => __( 'Please wait before trying again.', 'infosecnexus' ) ), 429 );
		}

		$honeypot = (string) $request->get_param( 'website' );
		if ( '' !== $honeypot ) {
			return new WP_REST_Response( array( 'message' => __( 'Thanks.', 'infosecnexus' ) ) );
		}

		$email = sanitize_email( (string) $request->get_param( 'email' ) );
		if ( ! is_email( $email ) ) {
			return new WP_REST_Response( array( 'message' => __( 'Enter a valid email address.', 'infosecnexus' ) ), 400 );
		}
		...
		return new WP_REST_Response( array( 'message' => __( 'You are on the list.', 'infosecnexus' ) ) );
	}
	...
	public static function render_form(): void {
		...
		<form class="newsletter-card" data-isnx-newsletter>
			...
			<label>
				...
				<input type="email" name="email" required placeholder="<?php esc_attr_e( 'you@example.com', 'infosecnexus' ); ?>">
			</label>
