...
final class Search {
	...
	public static function search( WP_REST_Request $request ): WP_REST_Response {
		if ( ! rate_limit( 'live_search', 40, 60 ) ) {
			return new WP_REST_Response( array( 'message' => __( 'Too many requests.', 'infosecnexus-toolkit' ) ), 429 );
		}
	...
	public static function modal(): void {
		...
		<div class="isnx-live-search" data-isnx-live-search hidden>
			<div class="isnx-live-search__panel" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Live search', 'infosecnexus-toolkit' ); ?>">
				<button type="button" class="isnx-live-search__close" data-isnx-live-search-close aria-label="<?php esc_attr_e( 'Close search', 'infosecnexus-toolkit' ); ?>">ÃƒÆ’Ã¢â‚¬â€</button>
				<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" data-isnx-live-search-form>
					<label>
						<span class="screen-reader-text"><?php esc_html_e( 'Search', 'infosecnexus-toolkit' ); ?></span>
						<input type="search" name="s" autocomplete="off" data-isnx-live-search-input placeholder="<?php esc_attr_e( 'Search security briefings...', 'infosecnexus-toolkit' ); ?>">
					</label>
					<button type="submit"><?php esc_html_e( 'Search', 'infosecnexus-toolkit' ); ?></button>
				</form>
